...
import { METHOD, OrderDetail, Product } from "@/lib/utils/types";
...
const EditOrder = ({
  ...
  const mutation = useMutation({
    mutationFn: async (reqData: TProductFormValues) => {
      ...
      const response = await fetcher(`/order`, METHOD.PUT, {
  ...
  const { data: dataOrder, isSuccess } = useQuery({
    queryKey: ["dataOrder"],
    queryFn: async () => {
      const response: OrderDetail = await fetcher(
        `/order/${params.slugs[0]}`,
        METHOD.GET
      );
      return await response;
    },
  ...
  useEffect(() => {
    if (isSuccess) {
      setValue("customer_name", dataOrder.customer_name);
      setValue(
        "products",
        dataOrder?.products.map((product) => ({
          product_name: product.product.id.toString(),
